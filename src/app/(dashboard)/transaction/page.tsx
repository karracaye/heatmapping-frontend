...
const Transaction = () => {
  ...
  return (
    <>
      <Template>
        ...
        <div className="w-full h-8 absolute bottom-0 flex items-center justify-center gap-3 rounded-b-[10px]">
          <p className="text-xs font-normal">
            {totalBeneficiaries
              ? `${pageIndex + 1} - ${
                  pageIndex + 20 > totalBeneficiaries
                    ? totalBeneficiaries
                    : pageIndex + 20
                } of ${totalBeneficiaries}`
              : "0 - 0 of 0"}
          </p>
          <button
          ...
          >
            <img
              ...
              alt=""
              className={`${
                pageIndex + 20 > totalBeneficiaries ? "opacity-50" : ""
              } h-5`}
            />
          </button>
        </div>
      ...
      <AddNewModal addNew={addNew} setAddNew={setAddNew} />
      {importModalOpen && (
        <ImportModal setImportModalOpen={setImportModalOpen} />
      )}
    </>
  );
};
