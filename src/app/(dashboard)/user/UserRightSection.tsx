        ...
        <div className="relative max-h-[100%] min-h-[80vh] bg-white rounded-[10px] shadow-[0_0_1px_2.9px_rgba(0,0,0,0.03)]">
            <div className="flex justify-between items-center pl-14 pr-9 pt-5 pb-2">
                ...
                <div className="h-[54px] flex items-center gap-2">
                      <div>
                            <button onClick={addNewClick} className='bg-[#303079] rounded-[10px] font-medium text-xs text-white h-[40px] px-4 hover:opacity-10 transition duration-300 ease-in-out'>Add New User</button>
                      </div>
                      <div className='relative '>
                            <input className='border-[#0000001a] border-[1px] h-[40px] w-[240px] rounded-[10px] font-normal text-[12px] pl-[30px]' type="text" placeholder='Who are you looking for?'/>
            ...
            <table className="w-full table-auto">
                ...
                {records.map((user, index) => (
                  <tr key={index} className="border-b text-left">
                      <td className="pr-3 px-6 pl-[40px] w-[30%]">{user.fullname}</td>
                      ...
                      <td className="py-3 px-6 flex items-center">
                        ...
                        <p className='font-normal text-[15px]'>{user.status === "active" ? "Active" : "Deactivated"}</p>
                      </td>
            ...
            {dataValue.length > recordPerPage && 
            <div className="w-[200px] absolute bottom-[10px] left-[42%] ">
              <div className="flex flex-row justify-center items-center">
                <p className="font-normal text-xs">{firstIndex + 1}-{lastIndex} of {totalNumber}</p>
